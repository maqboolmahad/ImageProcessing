...
def mirror(raw: List[List[List[int]]])-> None:
    ...
    for i in range(len(raw)):
        for j in range(len(raw[i])//2):
            raw[i][j], raw[i][-j-1] = raw[i][-j-1], raw[i][j]


def grey(raw: List[List[List[int]]])-> None:
    """
    Assume raw is image data. Modifies raw "averaging out" each
    pixel of raw. Specificall, for each pixel it totals the RGB
    ...
    for i in range(len(raw)):
        for j in range(len(raw[i])):
            sum = (raw[i][j][0] + raw[i][j][1] + raw[i][j][2]) // 3
            raw[i][j][0] = sum
            raw[i][j][1] = sum
            raw[i][j][2] = sum
...
def invert(raw: List[List[List[int]]])->None:
    ...
    for i in range(len(raw)):
        for j in range(len(raw[i])):
            ...
            for k in range(3):
                if raw[i][j][k] > raw[i][j][max]:
                    max = k
                if raw[i][j][k] < raw[i][j][min]:
                    min = k
            raw[i][j][max], raw[i][j][min] = raw[i][j][min], raw[i][j][max]
            L = [0,1,2]
            L.remove(max)
            if min in L:
                L.remove(min)
                if raw[i][j][L[0]] == raw[i][j][min]:
                    raw[i][j][L[0]] = raw[i][j][max]
                elif raw[i][j][L[0]] == raw[i][j][max]:
                    raw[i][j][L[0]] = raw[i][j][min]
...
def compress(raw: List[List[List[int]]])-> List[List[List[int]]]:
    ...
    [[[108, 77, 57], [255, 177, 50]],
     [[117, 166, 80], [0, 1, 1]]]
    """
    #L[0][0][0] + L[0][1][0] + L[1][0][0] + L[1][1][0]
    #L[0][2][0] + L[0][3][0] + L[1][2][0] + L[1][3][0]
    #L[2][0][0] + L[2][1][0] + L[3][0][0] + L[3][1][0]
    #L[2][2][0] + L[2][3][0] + L[3][2][0] + L[3][3][0]
    result = []
